...
public class ObjectTest {
		...
		// 복제를 위한 getClone 메소드를 구현했기 때문에 복제 개체를 얻어낼 수 있다. 
		...
		// s1과 s2는 같은 객체인가?
		System.out.println(s1 == s2);	// s2는 복제가 된 새 객체
	
		// s2 내부의 참조자료(배열)의 값을 바꿔보기
		s2.getScores()[2] = 100;
		System.out.println("s2=" + s2);
		System.out.println("s1=" + s1);
		// 얕은 복제의 겨우, 참조자료형은 주소값만 복제되기 때문에 문제가 발생할 수 있다.
		// 내부 참조 자료형의 경우 내부에서 Clone이 발생할 때 
		// 참조 자료형을 모두 새로 복제해서 다시 참조해야한다. 
		// *** 깊은 복제 ***
