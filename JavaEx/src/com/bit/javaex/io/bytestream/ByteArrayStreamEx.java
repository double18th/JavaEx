...
public class ByteArrayStreamEx {
	...
	public static void main (String[] args) {
		...
		// Stream은 일방향이다 (input이거나 output)m 
		...
		try {
			...
			// InputStream은 바이트 스트림을 입력받기 위한 최상위 추상 클래스(= 객체 생성 x)
			// 실제 인풋은 InputStream을 상속받은 실제 객체가 수행한다
			...
			//	IOException 처리 (예외처리 필수)
			System.err.println("복사 에러 : " + e.getMessage());			
		} catch (Exception e) {
			// 위에서 처리되지 않은 예외 여기서 처리
			e.printStackTrace();
		}
	}
}
